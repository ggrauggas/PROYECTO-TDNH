...
class UserModel {
  ...
  async create(userData) {
    const { username, email, password, full_name, diabetes_type, diagnosis_date, bio, glucose_enabled, notifications_enabled } = userData;

    // Hash de la contraseña
    const hashedPassword = await bcrypt.hash(password, 10);

    const query = `
      INSERT INTO users (username, email, password, full_name, diabetes_type, diagnosis_date, bio, glucose_enabled, notifications_enabled)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
      RETURNING id, username, email, full_name, role, glucose_enabled, notifications_enabled, created_at
    `;

    const values = [username, email, hashedPassword, full_name, diabetes_type, diagnosis_date, bio, glucose_enabled ?? false, notifications_enabled ?? false];
  ...
  async update(id, userData) {
    const { full_name, diabetes_type, diagnosis_date, bio, avatar_url, glucose_enabled, notifications_enabled, hashedPassword } = userData;
    ...
    const query = `
      ...
      SET full_name = COALESCE($1, full_name),
          ...
          updated_at = CURRENT_TIMESTAMP
      WHERE id = $9
      RETURNING id, username, email, full_name, diabetes_type, diagnosis_date, bio, avatar_url, glucose_enabled, notifications_enabled
    `;

    const values = [full_name, diabetes_type, diagnosis_date, bio, avatar_url, glucose_enabled ?? null, notifications_enabled ?? null, hashedPassword ?? null, id];
    const result = await pool.query(query, values);
    return result.rows[0];
  }
}
